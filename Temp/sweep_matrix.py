...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("-t", "--no-stream", action="store_true",
                   help="skip the stream_bw case (this Temp/ tree has no stream_bw)")
    ...
    p.add_argument("-p", "--no-spread", action="store_true",
                   help="skip rand+spread (access_mode=2) cases")
...
def build_jobs(args: argparse.Namespace) -> list[dict]:
    ...
    # samerow (mode 1, adjacent low columns) and spread (mode 2, far-apart high
    # columns) share the shape; they differ only in which column-step mask set the
    # binary uses (low vs high, high=True below), hence the max-lpa source.
    for enabled, access_mode, label, high in (
        (not args.no_samerow, 1, "samerow", False),
        (not args.no_spread,  2, "spread",  True),
    ):
        if not enabled:
            continue
        import address_mapping as am
        max_col_bits = len(am.col_step_masks(cfg.ADDR_MAP, high=high))
        max_lpa = 2 ** max_col_bits
        for lpa in LPA_VALUES:
            if lpa > max_lpa:
                print(f"[WARN] skipping {label} lines_per_access={lpa}: "
                      f"exceeds max columns ({max_lpa}) for ADDR_MAP={cfg.ADDR_MAP!r}",
                      file=sys.stderr)
                continue
            jobs.append({
                "pattern": "rand", "access_mode": access_mode, "mode_label": label,
                "lines_per_access": lpa, "binary": sbw.BINARY_RAND, "mode": "rand",
            })
...
def validate(jobs: list[dict]) -> None:
    ...
    if any(j["mode"] == "rand" for j in jobs):
        ...
        if any(j["access_mode"] in (1, 2) for j in jobs):
            import address_mapping as am
