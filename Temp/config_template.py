...
#   1 = samebank    : addr XOR'd with the LOW per-column-bit masks so all LINES_PER_ACCESS
#                     cachelines land in the same channel/rank/bank-group/bank/row,
#                     at ADJACENT columns (row-buffer-hit; lower column bits vary).
#   2 = spread      : like samebank but XOR'd with the HIGH per-column-bit masks, so the
#                     cachelines stay in the same channel/rank/bank-group/bank/row but land
#                     at FAR-APART columns (upper column bits vary).
#                     Modes 1 and 2 require LINES_PER_ACCESS <= max columns supported by ADDR_MAP below.
ACCESS_MODE = 0
