...
def run_benchmark(
    ...
    # samebank (access_mode=1) and spread (access_mode=2) pick their DRAM mapping
    # at runtime via ACCESS_MAP, so the sweep always uses config.ADDR_MAP regardless
    # of the binary's baked-in default — no recompile needed to switch mappings.
    env = dict(os.environ)
    pass_map = mode == "rand" and access_mode in (1, 2)
    if pass_map:
    ...
    if cfg.USE_SUDO:
        cmd.append("sudo")
        if pass_map:
            cmd.append("--preserve-env=ACCESS_MAP")
    cmd += [
        "numactl",
        "-C", cpulist,
        "-m", str(cfg.NUMA_NODE),
...
def main() -> None:
    ...
    if mode == "rand":
        ...
        if cfg.ACCESS_MODE not in (0, 1, 2):
            sys.exit(
                f"config.ACCESS_MODE={cfg.ACCESS_MODE} must be 0 (consecutive), "
                f"1 (samebank) or 2 (spread)."
            )
        if cfg.ACCESS_MODE in (1, 2):
    ...
    if mode == "rand":
        ...
        access_label = {0: "consecutive", 1: "samebank", 2: "spread"}[cfg.ACCESS_MODE]
        addr_map_note = f"  (addr_map={cfg.ADDR_MAP})" if cfg.ACCESS_MODE in (1, 2) else ""
