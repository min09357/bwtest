...
//                           adjacent columns (row-buffer-hit pattern). Requires
//                           lines_per_access <= 2**ACCESS_NUM_COL_BITS for the compiled-in
//                           ADDR_MAP (see access_masks.h, generated by gen_access_masks.py).
...
int main(int argc, char *argv[]) {
    ...
    if (mode == 1 && lines_per_access > (1 << ACCESS_NUM_COL_BITS)) {
        std::fprintf(stderr,
            "error: lines_per_access=%d exceeds max %d supported by ADDR_MAP=%s "
            "(ACCESS_NUM_COL_BITS=%d); regenerate access_masks.h for a different mapping "
            "or reduce lines_per_access\n",
            lines_per_access, 1 << ACCESS_NUM_COL_BITS, ACCESS_MAP_NAME, ACCESS_NUM_COL_BITS);
        return 1;
    }
    ...
    if (mode == 1) {
        for (int d = 0; d < lines_per_access; d++) {
            uint64_t m = 0;
            for (int b = 0; b < ACCESS_NUM_COL_BITS; b++)
                if ((d >> b) & 1) m ^= ACCESS_COL_STEP_MASK[b];
            col_masks[d] = m;
        }
    }
    ...
    std::printf("ncores=%d  iters/thread=%lld  streams=%d  region=%d GB  lines/access=%d (%d B)  "
        ...
        mode == 1 ? "  addr_map=" : "", mode == 1 ? ACCESS_MAP_NAME : "",
        BW_SIMD_WIDTH_STR);
