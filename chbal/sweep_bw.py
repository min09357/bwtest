...
def run_benchmark(
    ...
    # Both binaries pick their DRAM mapping at runtime via ACCESS_MAP — samebank
    # for its column-step masks, the hugepage allocator for its channel masks —
    # so the sweep always passes config.ADDR_MAP regardless of the binary's
    # baked-in default; no recompile needed to switch mappings.
    env = dict(os.environ)
    env["ACCESS_MAP"] = cfg.ADDR_MAP
    cmd = []
    if cfg.USE_SUDO:
        cmd += ["sudo", "--preserve-env=ACCESS_MAP"]
    cmd += [
        "numactl",
        "-C", cpulist,
        "-m", str(cfg.NUMA_NODE),
    ...
    # Trailing hugepage-allocation flags: [balance] [print_addrs] on both binaries.
    cmd.append(str(int(cfg.BALANCE_HUGEPAGES)))
    cmd.append(str(int(cfg.PRINT_HUGEPAGE_ADDRS)))
