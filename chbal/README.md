...
> **`chbal/` 브랜치 사본**: 상위 디렉토리 프로젝트에서 갈라져 나온 실험본으로,
> 1GB hugepage를 **DRAM 채널에 고르게** 잡고 각 hugepage의 **물리주소를 출력**하는
> 기능이 추가되어 있습니다. 아래 [hugepage 채널 균등 할당](#hugepage-채널-균등-할당)
> 절을 참고하세요. 상위 디렉토리 원본은 변경하지 않았습니다.
...
| `hugepage_alloc.h` | — | 1GB hugepage 할당기. 채널 클래스가 균등해지도록 물리 페이지를 골라 VA 순서로 인터리브 배치하고 물리주소를 출력 ([아래 절](#hugepage-채널-균등-할당)) |
...
./randread_bw [ncores] [iters_per_thread] [hugepages_1gb] [lines_per_access] [mode] [balance] [print_addrs]

# 순차 read (동일 인터페이스, lines_per_access/mode 없음)
./stream_bw [ncores] [iters_per_thread] [hugepages_1gb] [balance] [print_addrs]
...
- `balance` (기본 0): `1`이면 채널 클래스가 균등해지도록 물리 hugepage를 골라 VA 순서로 번갈아 배치 (root 필요, 아래 절 참고).
- `print_addrs` (기본 1): hugepage별 가상/물리 주소와 채널 클래스 표를 출력 (물리주소는 root일 때만).
...
채널 셀렉터 XOR 함수의 set 비트가 **전부 bit30 이상**이면 그 함수는 1GB hugepage
안에서 값이 변하지 않습니다. 예를 들어 `cascade_4ch_1dpc_2rank_32gb`의

```
channel[0] = 0x100          (bit 8)  → 페이지 안에서 토글됨
channel[1] = 0x1000000000   (bit 36) → 페이지 안에서 상수
```

이므로 hugepage 하나는 4채널 중 **2채널만** 사용합니다. 기존처럼
`mmap(MAP_HUGETLB)` 한 번으로 영역을 잡으면 커널 free list가 주는 물리 페이지를
그대로 받기 때문에, 16개 페이지가 전부 dram bit36의 한쪽(예: 64GB 미만)으로 몰리면
벤치마크가 실제로는 절반의 메모리 컨트롤러만 두드리게 됩니다.

### 동작

`balance=1`이면 [hugepage_alloc.h](hugepage_alloc.h)의 `hp_alloc()`이:

1. `memfd_create(MFD_HUGETLB|MFD_HUGE_1GB)`로 hugetlb 파일을 만들고,
2. 후보 페이지를 하나씩 폴트시켜 `/proc/self/pagemap`으로 물리주소를 읽어
   **채널 클래스**(페이지 상수 채널 함수들의 parity)로 분류하고,
   클래스별 정원이 차는 즉시 탐색을 멈춥니다 (페이지당 1GB 커널 zeroing 비용 절약).
3. 클래스를 라운드로빈으로 골라 `MAP_FIXED`로 연속 VA에 배치합니다 —
   **VA slot i는 클래스 `i % K`** 이므로 VA 순서로 채널이 번갈아 나타납니다.
4. 탐색만 하고 쓰지 않은 페이지는 `fallocate(PUNCH_HOLE)`로 풀에 반납합니다.

`stream_bw`는 스레드마다 연속 구간을 스캔하므로 이 VA 인터리브가 곧 스레드 간
채널 분산이 됩니다. `randread_bw`는 영역 전체를 무작위 접근하므로 "선택된 페이지
집합이 균등한 것" 자체가 핵심입니다.

탐색 상한은 현재 mempolicy(`numactl -m N`)가 허용하는 노드들의
`free_hugepages` 합입니다 — hugetlb 예약은 전역이지만 실제 페이지는 바인딩된
노드에서 나오므로, 무작정 많이 잡으면 폴트에서 SIGBUS가 날 수 있기 때문입니다.

### 제약과 폴백

- **root 필요**: 비특권 프로세스는 pagemap에서 PFN이 0으로 읽힙니다
  (Linux 4.0 이후). `config.USE_SUDO = True` 또는 `sudo`로 실행하세요.
- 채널 함수가 전부 bit30 미만인 매핑(이 저장소의 나머지 전부)은 페이지 하나가 이미
  전 채널을 커버하므로 균등화가 불필요하며, 그 사실을 알리고 기존 경로로 진행합니다.
- DRAM 홀 크기가 1GB의 배수가 아니면 DRAM 주소 공간에서 페이지 정렬이 깨지므로
  균등화를 건너뜁니다.
- 클래스별 후보가 모자라면 경고 후 가능한 최선의 조합으로 진행합니다.
- 위 어느 경우든, 그리고 memfd/hugetlb 실패 시에도 기존
  `mmap(MAP_PRIVATE|MAP_ANONYMOUS|MAP_HUGETLB)` 경로로 폴백합니다.

### 출력 예

```
hugepages: 4 x 1GB  map=cascade_4ch_1dpc_2rank_32gb  balance=on  classes=2  page-constant: ch_fn[0]=0x1000000000
 idx  vaddr              phys               dram               ch_class
 0    0x00007fa500000000 0x0000001040000000 0x0000000fc0000000 0
 1    0x00007fa540000000 0x0000002080000000 0x0000002000000000 1
 ...
 class histogram: [0]=2 [1]=2   balanced=yes
```

`config.py`에서는 `BALANCE_HUGEPAGES` / `PRINT_HUGEPAGE_ADDRS`로 제어하며,
`sweep_bw.py`가 두 바이너리 인자 끝에 자동으로 전달합니다.
