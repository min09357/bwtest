...
//
// Usage: ./stream_bw [ncores] [iters_per_thread] [hugepages_1gb] [balance] [print_addrs]
...
//       hugepage_alloc.h) — with contiguous per-thread sub-ranges that spreads the
//       threads over all channels. print_addrs=1 dumps each hugepage's physical address.
//       The DRAM mapping is picked at runtime via the ACCESS_MAP env var (unset ->
//       the compiled-in DECODE_DEFAULT_MAP == config.ADDR_MAP).
...
int main(int argc, char *argv[]) {
    ...
    bool    balance          = (argc > 4) ? std::atoi(argv[4]) != 0 : false;
    bool    print_addrs      = (argc > 5) ? std::atoi(argv[5]) != 0 : true;
    ...
    // Channel masks for the hugepage allocator (decode_masks.h), selected by the
    // same ACCESS_MAP env var randread_bw uses.
