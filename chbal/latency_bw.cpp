...
struct AddrInfo {
    uint64_t vaddr;
    uint64_t phys;  // raw system physical address, as reported by pagemap
    uint64_t dram;  // phys with the MMIO hole removed — the one to decode
    Decoded  d;
};
...
int main(int argc, char *argv[]) {
    ...
    if (DECODE_HOLE_SIZE)
        std::printf("dram hole=[0x%llx,0x%llx) size=%.2f GB — subtracted from phys before decode\n\n",
            (unsigned long long)DECODE_HOLE_START, (unsigned long long)DECODE_HOLE_END,
            (double)DECODE_HOLE_SIZE / (1024.0 * 1024.0 * 1024.0));
    else
        std::printf("dram hole=none — decoding raw physical addresses\n\n");
    ...
    auto run_category = [&](const char *label, bool same_bank_mode) -> double {
        ...
        while (found < pairs && budget > 0) {
            ...
            for (int t = 0; t < 200000 && budget > 0; t++, budget--) {
                ...
                bool match = same_bank_mode ? same_bank_diff_row(*sel, A.dram, cand.dram)
                                            : diff_bank_group(*sel, A.dram, cand.dram);
                if (!match) continue;
