...
// Uses HPCC RandomAccess POLY LFSR addressing with one LFSR stream per thread;
// per-thread skip-ahead keeps thread sequences disjoint.
//
...
//       balance=1 picks physical hugepages so every page-constant channel class is
//       equally represented and interleaved in VA order (needs root; see
//       hugepage_alloc.h). print_addrs=1 dumps each hugepage's physical address.
...
static void thread_func(ThreadArg &a, std::barrier<> &bar) {
    ...
    // One LFSR stream per thread; skip-ahead so each thread covers a
    // disjoint range of the sequence: thread tid starts at tid*L.
    uint64_t ran = hpcc_starts(static_cast<int64_t>(a.tid) * a.L);
    ...
    for (int64_t n = 0; n < L; n++) {
        ...
        uint64_t mv = static_cast<uint64_t>(0) - (ran >> 63);
        ran = (ran << 1) ^ (mv & POLY);
...
int main(int argc, char *argv[]) {
    ...
    bool    balance          = (argc > 6) ? std::atoi(argv[6]) != 0 : false;
    bool    print_addrs      = (argc > 7) ? std::atoi(argv[7]) != 0 : true;
    ...
    if (mode == 1) {
        ...
        // Page reordering only preserves the same-bank property if every column
        // step stays inside one 1GB page: a mask bit >= 30 would XOR the offset
        // into a different hugepage, whose physical page is unrelated.
        for (int b = 0; b < sel->num_col_bits; b++)
            if (sel->col_step_mask[b] >> 30)
                std::fprintf(stderr,
                    "warning: col_step_mask[%d]=0x%llx crosses the 1GB page boundary — "
                    "samebank addressing is not meaningful across hugepages\n",
                    b, (unsigned long long)sel->col_step_mask[b]);
    }

    // Channel masks for the hugepage allocator come from decode_masks.h and are
    // selected by the same ACCESS_MAP env var, independently of `mode`.
    const char *map_name = std::getenv("ACCESS_MAP");
    if (!map_name || !*map_name) map_name = DECODE_DEFAULT_MAP;
    const DecodeMap *dmap = nullptr;
    for (int i = 0; i < DECODE_NUM_MAPS; i++)
        if (std::strcmp(DECODE_MAPS[i].name, map_name) == 0) { dmap = &DECODE_MAPS[i]; break; }
    if (!dmap) {
        std::fprintf(stderr, "error: ACCESS_MAP=%s unknown; available:", map_name);
        for (int i = 0; i < DECODE_NUM_MAPS; i++)
    ...
    // Round to lines_per_access so each thread does L complete block accesses.
    int64_t round = lines_per_access;
    ...
    // Allocate hugepages_1gb × 1GB hugepages (optionally channel-balanced).
    HugeRegion region = hp_alloc(hugepages_1gb, *dmap, balance, print_addrs);
    if (!region.base) {
    ...
    // Flush the warmed region from all caches so the read measurement starts
    // cold. The working set already dwarfs L3, but an explicit flush removes
    // the ~L3-sized warm tail the memset leaves resident.
    std::printf("Flushing caches ... ");
    std::fflush(stdout);
    for (uint64_t off = 0; off < total; off += 64) {
        void *p = static_cast<uint8_t *>(base_ptr) + off;
    ...
    _mm_mfence();  // ensure all flushes retire before timing starts
    std::printf("done\n");
